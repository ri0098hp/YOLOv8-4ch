...
class Results(SimpleClass):
    ...
    def plot(
        ...
        **kwargs,  # deprecated args TODO: remove support in 8.2
        ...
        names = self.names
        annotator = Annotator(
            deepcopy(self.orig_img if img is None else img), line_width, font_size, font, pil, example=names
        )
    ...
    def save_crop(self, save_dir, file_name=Path("im.jpg")):
        ...
        for d in self.boxes:
            save_one_box(
                d.xyxy, self.orig_img.copy(), file=save_dir / self.names[int(d.cls)] / f"{file_name.stem}.jpg", BGR=True
            )
