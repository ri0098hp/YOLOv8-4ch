...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128, 128), txt_color=(255, 255, 255, 255), rotated=False):
    ...
    def text(self, xy, text, txt_color=(255, 255, 255, 255), anchor="top", box_style=False):
...
def plot_images(
    ...
    for i in range(bs):
        ...
        if paths:
            annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=txt_color)  # filenames
        if len(cls) > 0:
            idx = batch_idx == i
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    for m in ["Detect", "Pose", "Segment"]:
        ...
        if height > 1 and width > 1:
            sub_dir = save_dir / f"stage{stage}_{module_type.split('.')[-1]}"
            ...
            for i in range(n):
                fig, ax = plt.subplots()
                f = sub_dir / f"fillter_at_{i}.png"  # filename
            ...
            LOGGER.info(f"Saving {sub_dir}... ({n}/{channels})")
