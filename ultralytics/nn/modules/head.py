    ...
    def _build_input_proj_layer(self, ch):
        ...
        for in_channels in ch:
            self.input_proj.append(
                nn.Sequential(
                    nn.Conv2d(in_channels, self.hidden_dim, kernel_size=1, bias=False), nn.BatchNorm2d(self.hidden_dim)
                )
            )
        in_channels = ch[-1]
    ...
    def _generate_anchors(self, spatial_shapes, grid_size=0.05, dtype=torch.float32, device="cpu", eps=1e-2):
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            grid_y, grid_x = torch.meshgrid(
                torch.arange(end=h, dtype=torch.float32), torch.arange(end=w, dtype=torch.float32), indexing="ij"
            )
